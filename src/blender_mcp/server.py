...
class JJBlenderBridge:
    ...
    def _recv_response(self, timeout=180.0) -> bytes:
    ...
    def send(self, cmd_type: str, params: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        try:
            logger.info(f"Sending command: {cmd_type} with params: {params}")
...
_bridge: Optional[JJBlenderBridge] = None


def get_bridge() -> JJBlenderBridge:
    global _bridge
    ...
    _bridge = JJBlenderBridge()
    if not _bridge.connect():
        _bridge = None
        raise Exception("Could not connect to Blender. Make sure the Blender MCP addon is running.")
...
async def lifespan(server: FastMCP) -> AsyncIterator[Dict[str, Any]]:
    ...
    try:
        ...
        if _bridge:
            _bridge.disconnect()
            _bridge = None
        logger.info("Blender MCP server shut down")


mcp = FastMCP("BlenderMCP", lifespan=lifespan)


@mcp.tool()
def ping(ctx: Context) -> str:
    """Check connectivity with the Blender addon."""
    result = get_bridge().send("ping")
...
def get_scene_info(ctx: Context) -> str:
    ...
    try:
        result = get_bridge().send("get_scene_info")
        return json.dumps(result, indent=2)
    except Exception as e:
...
def list_objects(ctx: Context, type_filter: str = None) -> str:
    ...
    try:
        ...
        result = get_bridge().send("list_objects", params)
        return json.dumps(result, indent=2)
    except Exception as e:
...
def get_object_info(ctx: Context, object_name: str) -> str:
    ...
    """
    try:
        result = get_bridge().send("get_object_info", {"name": object_name})
        return json.dumps(result, indent=2)
    except Exception as e:
...
def get_viewport_screenshot(ctx: Context, max_size: int = 800) -> Image:
    ...
    try:
        temp_path = os.path.join(
            tempfile.gettempdir(),
            f"jj_blender_mcp_{os.getpid()}.png",
        )
        result = get_bridge().send("get_viewport_screenshot", {
            "filepath": temp_path,
            "max_size": max_size,
            "format": "png",
        })
...
def execute_blender_code(ctx: Context, code: str) -> str:
    ...
    """
    try:
        result = get_bridge().send("execute_code", {"code": code})
